/* ======== Example sumOf =================
Let's write an algorithm that calculates the sum of all the numbers in an array. You can think of solving this problem using the following steps:
...
function reverseString(str) {
  ...
  // substring extracts charatcter from string with index starting at 0, and returns a new substring
  // charAt returns character of a string at an index
...
function stringSplitter(str, splitter) {
  ...
  if (str.charAt(0) === splitter) {
    splitArray.push(splitStr);
    splitStr = "";
  }
  return (splitStr += str.charAt(0)), stringSplitter(str.slice(1), splitter);
}
